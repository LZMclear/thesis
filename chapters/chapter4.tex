...
CF-mMIMO系统具有大量天线和空间稀疏部署的特点，有望为海量设备提供超可靠和低延迟通信(URLLC)服务。然而，这种方法的实施需要大量的部署成本，并且不可避免地需要使用非理想的硬件。本文针对非理想硬件条件下CF-mMIMO系统的URLLC性能优化问题，构建了非线性PA与低分辨率ADC的混合损伤模型，推导了上行链路UE速率闭合表达式。为降低传统优化算法的计算复杂度，提出基于GNN与DRL融合的智能功率控制策略，通过图嵌入技术建模用户空间相关性，动态抑制硬件损伤与时变信道干扰。所提方案在保持毫秒级时延的同时，为工业自动化等物联网应用提供理论与技术支撑。
